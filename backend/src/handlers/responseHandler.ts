...
const parseShowVersion = (text: string) => {
 ...
 */
const responseHandler = async (serialNumber: string, data: CommandResponsePayload) => {
  try {
    ...
    if (data.status === 4) {
      const progressData = data.data as { progress?: number } | null;
      const progressValue =
        progressData && typeof progressData === "object" && "progress" in progressData
    ...
    await prisma.commandLog.update({
      ...
      data: {
        responseStatus: data.status,
        responseMessage: data.message,
        responseData: data.data as object ?? {},
        progress: data.status === 0 ? "completed" : "failed",
      },
    });       
 
    console.log(
      `[response] CommandLog updated for ${data.request_id} — status: ${data.status}, message: ${data.message}`
    );
    ...
    if (existing.command === "show version" && data.status === 0 && typeof data.data === "string") {
      ...
      if (parsed.model) updateData.model = parsed.model;
      if (parsed.macAddress) updateData.macAddress = parsed.macAddress;
      if (parsed.hardwareVersion) updateData.hardwareVersion = parsed.hardwareVersion;
      if (parsed.softwareVersion) updateData.softwareVersion = parsed.softwareVersion;
