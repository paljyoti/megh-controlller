...
const VALID_KINDS = ["svi", "routedPort"] as const;
type InterfaceKind = (typeof VALID_KINDS)[number];

const targetFor = (row: { interfaceKind: string; vlanId: number | null; port: string | null }): InterfaceTarget =>
  row.interfaceKind === "svi"
    ? { kind: "svi", vlanId: row.vlanId as number }
    : { kind: "single", port: row.port as string };

// GET /api/v1/device/:id/l3-interfaces
export const listL3Interfaces = asyncHandlers(async (req: newReq, res: Response) => {
...
export const setL3Address = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
    ...
    return res
      ...
      .json(
        new ApiResponse(
          400,
          {},
          "confirm must be true — setting an L3 address can change the device's management reachability"
        )
      );

  const interfaceKind = body.interfaceKind;
  if (!VALID_KINDS.includes(interfaceKind as InterfaceKind))
  ...
  const runStep = async (field: string, spec: { command: string; mode: "config" | "exec"; params?: Record<string, unknown> }) => {
    try {
  ...
  let ipv6Ok = false;
  if (canProceedWithIp) {
    if (ipv4Address) ipv4Ok = await runStep("ipv4Address", commands.l3.setIpv4(target, ipv4Address));
    if (ipv6Address) ipv6Ok = await runStep("ipv6Address", commands.l3.setIpv6(target, ipv6Address));
    ...
    if (ipv4Address) results.ipv4Address = { applied: false, message: "Skipped — 'no switchport' was rejected" };
    if (ipv6Address) results.ipv6Address = { applied: false, message: "Skipped — 'no switchport' was rejected" };
    anyFailed = true;
  }

  const dbData: Record<string, unknown> = { interfaceKind, interfaceName, vlanId, port };
  if (ipv4Ok) dbData.ipv4Address = ipv4Address;
  if (ipv6Ok) dbData.ipv6Address = ipv6Address;
  ...
  const row = await prisma.l3Interface.upsert({
    ...
    create: {
      deviceId: device.id,
      interfaceKind: interfaceKind as InterfaceKind,
      interfaceName,
      vlanId,
      port,
  ...
  return res
    ...
    .json(
      new ApiResponse(
        statusCode,
        { l3Interface: row, results },
        anyFailed ? "Some L3 address fields were rejected by the device" : "L3 address configured"
      )
    );
});
...
export const deleteL3Address = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  const l3Id = req.params.l3Id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  if (!l3Id) return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: l3Id"));
  ...
  if (!existing) return res.status(404).json(new ApiResponse(404, {}, "L3 interface not found on this device"));
  ...
  if (existing.ipv4Address) await runStep("ipv4Address", commands.l3.clearIpv4(target, existing.ipv4Address));
  if (existing.ipv6Address) await runStep("ipv6Address", commands.l3.clearIpv6(target, existing.ipv6Address));
  if (existing.interfaceKind === "routedPort" && !anyFailed) await runStep("routedMode", commands.l3.clearRoutedMode(target));
