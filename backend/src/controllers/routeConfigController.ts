...
const isValidIpSegment = (v: string): boolean => {
...
interface RouteBody {
  isDefaultRoute?: unknown;
  destIpSegment?: unknown;
  destIpMask?: unknown;
  interfaceType?: unknown;
  forwardingRoutingAddress?: unknown;
  distanceMetric?: unknown;
  routingTag?: unknown;
  description?: unknown;
}

const parseRouteBody = (body: RouteBody) => {
  ...
  const interfaceType =
    typeof body.interfaceType === "string" && body.interfaceType.trim()
      ? body.interfaceType.trim()
      : "null0 interface";

  if (!isValidIpSegment(destIpSegment)) return { error: "Dest IP Segment must be a valid IP" };
  if (!isValidIpSegment(destIpMask)) return { error: "Dest IP Mask must be a valid IP mask" };

  const forwardingRoutingAddress =
    typeof body.forwardingRoutingAddress === "string" && body.forwardingRoutingAddress.trim()
  ...
  let distanceMetric: number | null = null;
  ...
  let routingTag: string | null = null;
...
export const createRouteConfig = asyncHandlers(async (req: newReq, res: Response) => {
  ...
  const parsed = parseRouteBody(req.body as RouteBody);
...
export const deleteRouteConfig = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
...
export const listRouteConfigs = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
