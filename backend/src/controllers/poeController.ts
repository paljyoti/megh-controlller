...
export const getPoeSettings = asyncHandlers(async (req: newReq, res: Response) => {
...
export const updatePoePowerBudget = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  const watts = (req.body as { watts?: unknown })?.watts;
  ...
  try {
    ...
    if (response.status !== 0) {
      return res
        .status(422)
        .json(
          new ApiResponse(422, { requestId, switchStatus: response.status }, response.message || "Switch rejected the command")
        );
    }
...
export const updatePoeLegacyMode = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  const body = req.body as { enabled?: unknown; confirm?: unknown };
    ...
    return res
      ...
      .json(
        new ApiResponse(
          400,
          {},
          "confirm must be true to enable PoE legacy mode — the switch itself warns this can damage a connected device if the port isn't attached to a PD"
        )
      );
...
export const updatePoePowerAlarm = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  const percent = (req.body as { percent?: unknown })?.percent;
...
export const updatePoePowerReserved = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
