...
const sumBytes = (interfaceStats: { rxBytes: string; txBytes: string }[]) => {
  let rx = 0n;
  let tx = 0n;
...
export const getNetworkOverview = asyncHandlers(async (req: newReq, res: Response) => {
  ...
  const byDevice = new Map<string, typeof snapshots>();
  ...
  for (const deviceSnaps of byDevice.values()) {
    for (let i = 1; i < deviceSnaps.length; i++) {
      ...
      if (deltaSeconds <= 0) continue; // guard against out-of-order/duplicate timestamps
