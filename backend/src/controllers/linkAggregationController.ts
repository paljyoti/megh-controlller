...
const VALID_MODES = ["static", "active", "passive"] as const;
type LinkAggMode = (typeof VALID_MODES)[number];
...
interface LinkAggBody {
  groupId?: unknown;
  mode?: unknown;
  memberPorts?: unknown;
}

const parseLinkAggBody = (body: LinkAggBody) => {
  const groupId = Number(body.groupId);
  if (!Number.isInteger(groupId) || groupId < 1 || groupId > 12)
    return { error: "Aggregation Group must be an integer between 1 and 12" };

  const mode = VALID_MODES.includes(body.mode as LinkAggMode) ? (body.mode as LinkAggMode) : null;
  if (!mode) return { error: `Mode must be one of: ${VALID_MODES.join(", ")}` };

  const memberPorts = Array.isArray(body.memberPorts)
    ? body.memberPorts.filter((p): p is string => typeof p === "string" && p.trim().length > 0)
    : [];
  if (memberPorts.length === 0) return { error: "At least one member port is required" };
...
export const listLinkAggregations = asyncHandlers(async (req: newReq, res: Response) => {
  ...
  const upPorts = new Set(
    (latestTelemetry?.interfaceStats ?? []).filter((s) => s.status === "up").map((s) => s.port)
  );
...
export const createLinkAggregation = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  const parsed = parseLinkAggBody(req.body as LinkAggBody);
  ...
  return res
    ...
    .json(
      new ApiResponse(
        statusCode,
        { linkAggregation, results },
        anyFailed ? "Aggregation group created, but some member ports were rejected" : "Aggregation group created"
      )
    );
});
...
export const deleteLinkAggregation = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  const laId = req.params.laId;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  if (!laId) return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: laId"));
    ...
    return res
      ...
      .json(
        new ApiResponse(
          422,
          { results },
          "Some member ports could not be removed from the channel group; the group was not deleted"
        )
      );
  }
...
export const getLoadBalanceMethod = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { method: access.device.loadBalanceMethod }, "Load balance method fetched")
    );
});
...
export const updateLoadBalanceMethod = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  if (typeof method !== "string" || !LOAD_BALANCE_METHODS.includes(method as (typeof LOAD_BALANCE_METHODS)[number]))
    return res
      .status(400)
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, { method: updated.loadBalanceMethod, requestId }, "Load balance method updated")
      );
  } catch (err) {
    const requestId = err instanceof CommandTimeoutError ? err.requestId : undefined;
    return res
...
export const getLacpSystemPriority = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { priority: access.device.lacpSystemPriority }, "LACP system priority fetched")
    );
});
...
export const updateLacpSystemPriority = asyncHandlers(async (req: newReq, res: Response) => {
  const deviceParam = req.params.id;
  if (!deviceParam)
    return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, { priority: updated.lacpSystemPriority, requestId }, "LACP system priority updated")
      );
  } catch (err) {
    const requestId = err instanceof CommandTimeoutError ? err.requestId : undefined;
    return res
