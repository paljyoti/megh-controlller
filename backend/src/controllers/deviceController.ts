...
const getParam = (req: newReq, key: string): string => {
  const val = req.params[key];
  if (!val) throw new ApiError(400, `Missing route parameter: ${key}`);
  return val;
};

// Helper: verify device access based on user role   
const verifyDeviceAccess = async (req: newReq, deviceId: string) => {
  const device = await findDeviceById(deviceId);
  if (!device) throw new ApiError(404, "Device not found");
  if (!req.user) throw new ApiError(401, "unauthorized request");

  if (req.user.role === "SUPERADMIN") return device;
...
export const getTelemetry = asyncHandlers(async (req: newReq, res: Response) => {
  ...
  if (!latest) throw new ApiError(404, "No telemetry data found for this device");
...
export const getEvents = asyncHandlers(async (req: newReq, res: Response) => {
  const device = await verifyDeviceAccess(req, getParam(req, "id"));
...
export const getStatus = asyncHandlers(async (req: newReq, res: Response) => {
  const device = await verifyDeviceAccess(req, getParam(req, "id"));
...
export const getStatusHistory = asyncHandlers(async (req: newReq, res: Response) => {
  const device = await verifyDeviceAccess(req, getParam(req, "id"));
...
export const sendCommand = asyncHandlers(async (req: newReq, res: Response) => {
  const device = await verifyDeviceAccess(req, getParam(req, "id"));

  const { command, mode, params = {} } = req.body as {
    command: string;
    mode: CommandMode;
    params?: Record<string, unknown>;
  };
  if (!command) throw new ApiError(400, "command field is required");
  if (mode !== "exec" && mode !== "config") throw new ApiError(400, "mode must be 'exec' or 'config'");
...
export const broadcastCommand = asyncHandlers(async (req: newReq, res: Response) => {
  const { command, mode, params = {} } = req.body as {
    command: string;
    mode: CommandMode;
    params?: Record<string, unknown>;
...
export const fileTransfer = asyncHandlers(async (req: newReq, res: Response) => {
  const device = await verifyDeviceAccess(req, getParam(req, "id"));

  const { file_type, file_url, algorithm, checksum, version } = req.body as {
    file_type: FileType;
    file_url: string;
  ...
  const validAlgorithms: ChecksumAlgorithm[] = ["MD5", "SHA1", "SHA256", "SHA512"];
  ...
  const payload: FileTransferPayload = version
...
export const getCommandStatus = asyncHandlers(async (req: newReq, res: Response) => {
  ...
  if (!log) throw new ApiError(404, "Command log not found");
...
export const getUnassignedDevices = asyncHandlers(async (req: newReq, res: Response) => {
  if (!req.user) throw new ApiError(401, "unauthorized request");
...
export const assignDevice = asyncHandlers(async (req: newReq, res: Response) => {
  if (!req.user) throw new ApiError(401, "unauthorized request");
  if (req.user.role === "USER") throw new ApiError(403, "Users cannot assign devices");
  ...
  if (req.user.role === "SUPERADMIN") {
    if (!organizationId) throw new ApiError(400, "organizationId required for SUPERADMIN assignment");
  ...
  // ADMIN flow
  if (!req.user.orgs?.id) throw new ApiError(400, "Admin has no organization");

  if (device.organizationId === null) {
    const data: Record<string, string | null> = { organizationId: req.user.orgs.id };
    if (userId) {
      const targetUser = await prisma.user.findUnique({ where: { id: userId } });
  ...
  if (device.organizationId === req.user.orgs.id) {
    if (!userId) throw new ApiError(400, "userId required to reassign within org");
    const targetUser = await prisma.user.findUnique({ where: { id: userId } });
    if (!targetUser || targetUser.organizationId !== req.user.orgs.id)
      throw new ApiError(400, "Target user not in your organization");
    const updated = await prisma.device.update({
...
export const unassignDevice = asyncHandlers(async (req: newReq, res: Response) => {
  if (!req.user) throw new ApiError(401, "unauthorized request");
  if (req.user.role === "USER") throw new ApiError(403, "Users cannot unassign devices");
