...
import type { CommandSpec } from "../commands/deviceCommands.js";
...
const VALID_MODES = ["stp", "rstp", "mstp"] as const;
type StpMode = (typeof VALID_MODES)[number];

// GET /api/v1/device/:id/stp
export const getStpSettings = asyncHandlers(async (req: newReq, res: Response) => {
...
export const updateStpMode = asyncHandlers(async (req: newReq, res: Response) => {
  const access = await verifyDeviceAccess(req, req.params.id!);
  if (!access.ok) return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
  const { device } = access;
  ...
  if (!VALID_MODES.includes(mode as StpMode))
  ...
  try {
    const { requestId, response } = await runDeviceCommand(device, {
      ...commands.stp!.setMode(mode as StpMode),
      logParams: { mode },
    });
    ...
    const updated = await prisma.device.update({ where: { id: device.id }, data: { stpMode: mode as StpMode, stpEnabled: false } });
...
export const updateStpEnabled = asyncHandlers(async (req: newReq, res: Response) => {
  const access = await verifyDeviceAccess(req, req.params.id!);
  if (!access.ok) return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
  const { device } = access;
  if (!requireSupport(device, res)) return;
...
export const updateStpTimers = asyncHandlers(async (req: newReq, res: Response) => {
  const access = await verifyDeviceAccess(req, req.params.id!);
  if (!access.ok) return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
  const { device } = access;
  if (!requireSupport(device, res)) return;

  const body = req.body as { priority?: unknown; helloTime?: unknown; forwardDelay?: unknown; maxAge?: unknown };
  ...
  const helloTime = (body.helloTime as number | undefined) ?? device.stpHelloTime;
  const forwardDelay = (body.forwardDelay as number | undefined) ?? device.stpForwardDelay;
  const maxAge = (body.maxAge as number | undefined) ?? device.stpMaxAge;
  ...
  const steps: { field: string; spec: CommandSpec; dbField: string; value: number }[] = [];
  if (body.priority !== undefined) steps.push({ field: "priority", spec: commands.stp!.setPriority(body.priority as number), dbField: "stpPriority", value: body.priority as number });
  if (body.helloTime !== undefined) steps.push({ field: "helloTime", spec: commands.stp!.setHelloTime(body.helloTime as number), dbField: "stpHelloTime", value: body.helloTime as number });
  if (body.forwardDelay !== undefined) steps.push({ field: "forwardDelay", spec: commands.stp!.setForwardDelay(body.forwardDelay as number), dbField: "stpForwardDelay", value: body.forwardDelay as number });
  if (body.maxAge !== undefined) steps.push({ field: "maxAge", spec: commands.stp!.setMaxAge(body.maxAge as number), dbField: "stpMaxAge", value: body.maxAge as number });

  if (steps.length === 0) return res.status(400).json(new ApiResponse(400, {}, "At least one of priority, helloTime, forwardDelay, maxAge is required"));
