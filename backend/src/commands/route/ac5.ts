import type { RouteCommandBuilder, RouteCommandParams } from "./types.js";
...
export const buildRouteCommand = ({
  ...
  distanceMetric,
}: RouteCommandParams): string => {
  const nextHop =
    interfaceType === "null0 interface" ? "null0" : (forwardingRoutingAddress || "").trim();
  const parts = ["ip route", destIpSegment, destIpMask, nextHop];
  if (distanceMetric) parts.push(String(distanceMetric));
  return parts.join(" ");
};

export const routeAc5: RouteCommandBuilder = {
