...
const MIN_VLAN = 1;
const MAX_VLAN = 4094;
export const isValidVlanNumber = (n) => Number.isInteger(n) && n >= MIN_VLAN && n <= MAX_VLAN;
...
export const parseVlanRange = (raw) => {
    const trimmed = raw.trim();
    if (/\s/.test(trimmed))
        return { error: "VLAN range must not contain spaces" };
    const rangeMatch = trimmed.match(/^(\d+)-(\d+)$/);
    if (rangeMatch) {
        const lo = Number(rangeMatch[1]);
        const hi = Number(rangeMatch[2]);
        if (lo >= hi)
            return { error: "VLAN range must have the lower number first, e.g. 2-10" };
    ...
    if (/^\d+$/.test(trimmed)) {
        ...
        if (!isValidVlanNumber(v))
            return { error: `VLAN ID must be between ${MIN_VLAN} and ${MAX_VLAN}` };
...
export const parseVlanList = (raw) => {
    const trimmed = raw.trim();
    if (trimmed === "all")
        return { value: { kind: "all" }, spec: "all" };
    if (trimmed === "none" || trimmed === "")
        return { value: { kind: "none" }, spec: "none" };
    if (/\s/.test(trimmed))
        return { error: "VLAN list must not contain spaces between comma-separated parameters" };
    const ids = new Set();
    for (const part of trimmed.split(",")) {
        if (part === "")
            return { error: "VLAN list has an empty entry (check for a trailing comma)" };
        const rangeMatch = part.match(/^(\d+)-(\d+)$/);
        if (rangeMatch) {
            const lo = Number(rangeMatch[1]);
            const hi = Number(rangeMatch[2]);
            if (lo >= hi)
                return { error: `Range "${part}" must have the lower number first` };
            if (!isValidVlanNumber(lo) || !isValidVlanNumber(hi))
                return { error: `VLAN IDs must be between ${MIN_VLAN} and ${MAX_VLAN}` };
            for (let v = lo; v <= hi; v++)
                ids.add(v);
...
//# sourceMappingURL=vlanList.js.map