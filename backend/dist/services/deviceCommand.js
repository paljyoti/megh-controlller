...
export const publishCommand = async (device, input) => {
    ...
    const log = await prisma.commandLog.create({
        data: { deviceId: device.id, requestId, command, params: (logParams ?? params) },
    });
...
export const runDeviceCommand = async (device, input, timeoutMs) => {
