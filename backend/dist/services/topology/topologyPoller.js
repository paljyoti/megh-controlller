...
export const refreshDeviceMacTable = async (device) => {
    ...
    const { response } = await runDeviceCommand(device, { ...commands.topology.showMacTable(), logParams: { source: "topologyPoller" } }, COMMAND_TIMEOUT_MS);
    ...
    await prisma.$transaction([
        ...rows.map((r) => prisma.macEntry.upsert({
            where: {
    ...
    try {
        ...
        for (const d of devices) {
            try {
                ...
                const msg = err instanceof CommandTimeoutError ? "timed out" : err.message;
...
//# sourceMappingURL=topologyPoller.js.map