...
const deviceWhereForUser = (req) => {
    ...
    if (req.user.role === "ADMIN")
        return { organizationId: req.user.orgs?.id ?? null };
    if (req.user.role === "USER")
        return { assignedToId: req.user.id };
    return { organizationId: { not: null } };
};
const findScopedDevice = async (req, id) => {
...
// GET /api/v1/topology
export const getTopology = asyncHandlers(async (req, res) => {
...
export const refreshTopology = asyncHandlers(async (req, res) => {
    ...
    for (const d of devices) {
        try {
            ...
            const error = err instanceof CommandTimeoutError ? "Device did not respond" : err.message;
...
export const getDeviceMacTable = asyncHandlers(async (req, res) => {
    const id = req.params.id;
    if (!id)
        throw new ApiError(400, "Missing route parameter: id");
    ...
    return res.status(200).json(new ApiResponse(200, {
        ...
        ports,
        updatedAt: entries.reduce((min, e) => (min === null || e.lastSeen < min ? e.lastSeen : min), null),
