...
export const createOrg = asyncHandlers(async (req, res) => {
    const user = req.user;
    if (!user) {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { orgs }, "organization is successfully created"));
});
export const getAllDepts = asyncHandlers(async (req, res) => {
    const user = req.user;
...
export const getAllOrgs = asyncHandlers(async (req, res) => {
...
export const getOrgUsers = asyncHandlers(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, "unauthorized request");
...
export const getOrgDetails = asyncHandlers(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, "unauthorized request");
