...
export const createPortSecurityMac = asyncHandlers(async (req, res) => {
    ...
    const { device } = access;
    const parsed = parseStaticMacBody(req.body);
    if ("error" in parsed)
...
export const deletePortSecurityMac = asyncHandlers(async (req, res) => {
    const deviceParam = req.params.id;
    ...
    if (device.status !== "online")
        return res
            ...
            .json(new ApiResponse(409, {}, `Device is ${device.status}; it must be online to configure port security`));
    try {
        const target = { kind: "single", port: existing.port };
        const { requestId, response } = await runDeviceCommand(device, {
...
export const listPortSecurityMacs = asyncHandlers(async (req, res) => {
    const deviceParam = req.params.id;
    if (!deviceParam)
        return res.status(400).json(new ApiResponse(400, {}, "Missing route parameter: id"));
    const access = await verifyDeviceAccess(req, deviceParam);
...
//# sourceMappingURL=portSecurityController.js.map