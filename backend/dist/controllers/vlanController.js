...
export const createVlan = asyncHandlers(async (req, res) => {
    ...
    if (existingVlans.length > 0)
        return res
            .status(409)
            .json(new ApiResponse(409, {}, `VLAN(s) already exist on this device: ${existingVlans.map((v) => v.vlanId).join(", ")}`));
    ...
    try {
        ...
        const vlans = await prisma.$transaction(ids.map((vlanId) => prisma.vlan.create({
...
export const deleteVlan = asyncHandlers(async (req, res) => {
    ...
    if (req.query.force === "true") {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, {}, ids.length === 1
