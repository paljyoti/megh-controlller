...
export const getLoopDetectSettings = asyncHandlers(async (req, res) => {
...
export const updateLoopDetectGlobal = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateLoopDetectInterval = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateLoopDetectTrap = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateErrdisableTimeout = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const recoverErrdisablePort = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
    ...
    if (typeof port !== "string" || !port.trim())
        return res.status(400).json(new ApiResponse(400, {}, "port is required"));
    if (!requireOnline(device, res))
        return;
    const commands = getDeviceCommands(device.model);
    try {
...
//# sourceMappingURL=loopDetectController.js.map