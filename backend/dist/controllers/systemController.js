...
const isValidIpv6 = (v) => IPV6_RE.test(v) && v.includes(":");
...
const runAndPersist = async (device, res, spec, logParams, dbUpdate) => {
    try {
...
export const getSystemSettings = asyncHandlers(async (req, res) => {
...
export const updateHostname = asyncHandlers(async (req, res) => {
    ...
    if (!requireOnline(device, res))
        return;
    return runAndPersist(device, res, hostname === null ? commands.system.clearHostname() : commands.system.setHostname(hostname.trim()), { field: "hostname", value: hostname }, { hostname: hostname === null ? null : hostname.trim() });
});
// PATCH /api/v1/device/:id/system/ntp — { ntpServer: string }
export const updateNtpServer = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateTimezone = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
const VALID_WEB_MODES = ["all", "http", "https"];
// PATCH /api/v1/device/:id/system/web-server — { mode: "all"|"http"|"https"|null }
export const updateWebServer = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
    const mode = req.body?.mode;
    if (mode !== null && !VALID_WEB_MODES.includes(mode))
        return res.status(400).json(new ApiResponse(400, {}, `mode must be one of: ${VALID_WEB_MODES.join(", ")}, or null to disable`));
    const commands = getDeviceCommands(device.model);
    if (!requireOnline(device, res))
        return;
    return runAndPersist(device, res, commands.system.setWebServer(mode), { field: "webServerMode", value: mode }, { webServerMode: mode });
});
// PATCH /api/v1/device/:id/system/telnet-server — { enabled: boolean }
export const updateTelnetServer = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateSshServer = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const updateManagementIp = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
    ...
    if (!requireOnline(device, res))
        return;
    return runAndPersist(device, res, body.mode === "static" ? commands.system.setManagementIpStatic(vlanId, ip, gateway) : commands.system.setManagementIpDhcp(vlanId), { field: "managementIp", vlanId, mode: body.mode, ip, gateway }, { managementVlanId: vlanId, managementIpMode: body.mode, managementIp: ip ?? null, managementGateway: gateway ?? null });
});
...
export const updateManagementIpv6 = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
    ...
    if (!requireOnline(device, res))
        return;
    return runAndPersist(device, res, body.mode === "static" ? commands.system.setManagementIpv6Static(vlanId, ip, gateway) : commands.system.setManagementIpv6Dhcp(vlanId), { field: "managementIpv6", vlanId, mode: body.mode, ip, gateway }, { managementVlanId: vlanId, managementIpv6Mode: body.mode, managementIpv6: ip ?? null, managementIpv6Gateway: gateway ?? null });
});
// POST /api/v1/device/:id/system/write — saves the running config so it survives a reboot.
export const writeConfig = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const reloadDevice = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
...
export const factoryRestore = asyncHandlers(async (req, res) => {
    const access = await verifyDeviceAccess(req, req.params.id);
    if (!access.ok)
        return res.status(access.status).json(new ApiResponse(access.status, {}, access.message));
    const { device } = access;
    ...
    try {
        ...
        if (copyResult.response.status !== 0) {
            return res
                .status(422)
                .json(new ApiResponse(422, { requestId: copyResult.requestId, switchStatus: copyResult.response.status }, copyResult.response.message || "Switch rejected the restore command"));
        }
        const reloadResult = await runDeviceCommand(device, { ...commands.system.reload(), logParams: {} });
        return res.status(200).json(new ApiResponse(200, { copyRequestId: copyResult.requestId, reloadRequestId: reloadResult.requestId }, "Factory restore initiated — the device is reloading"));
    }
    catch (err) {
        const requestId = err instanceof CommandTimeoutError ? err.requestId : undefined;
        return res.status(504).json(new ApiResponse(504, { requestId }, "Device did not respond in time"));
