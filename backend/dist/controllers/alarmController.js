...
export const getAllAlarms = asyncHandlers(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, { alarms, total, page, totalPages: Math.max(1, Math.ceil(total / limit)) }, "Alarms fetched"));
});
// GET /api/v1/alarm/summary — counts for dashboard cards
export const getAlarmSummary = asyncHandlers(async (req, res) => {
...
export const getDeviceAlarms = asyncHandlers(async (req, res) => {
...
export const acknowledgeAlarm = asyncHandlers(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, "unauthorized request");
...
export const resolveAlarm = asyncHandlers(async (req, res) => {
    const alarmId = req.params.id;
    if (!alarmId)
        throw new ApiError(400, "Missing route parameter: id");
    const alarm = await prisma.alarm.findUnique({ where: { id: alarmId } });
