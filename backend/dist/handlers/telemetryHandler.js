...
const evaluateThresholdAlarm = async (deviceId, type, value, unit) => {
...
const requestIdentityRecovery = async (device) => {
 ...
 */
const telemetryHandler = async (serialNumber, data) => {
    try {
        ...
        if (!device) {
            console.warn(`[telemetry] Device not found for serialNumber: ${serialNumber}. Creating new device.`);
        ...
        console.log(`[telemetry] Saved telemetry ${telemetry.id} for ${serialNumber} with ${data.interfaces?.length ?? 0} interfaces`);
        ...
        console.error(`[telemetry] Error saving telemetry for ${serialNumber}:`, err);
    }
};
