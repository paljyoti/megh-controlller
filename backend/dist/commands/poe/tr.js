...
export const poeTr = {
    ...
    setPowerAlarm: (percent) => percent === null
    ...
    setPortMaxPower: (target, watts) => watts === null
        ? buildInterfaceCommand(target, "no poe max-power")
        : buildInterfaceCommand(target, `poe max-power ${watts}`),
    setPortLegacyMode: (target, enabled) => buildInterfaceCommand(target, enabled ? "poe legacy" : "no poe legacy"),
    setPortPdDescription: (target, description) => description
    ...
    setPortPdDetectParams: (target, intervalSeconds, times) => buildInterfaceCommand(target, `poe pd-detect parameter interval ${intervalSeconds} times ${times}`),
