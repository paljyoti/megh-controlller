...
export const stpTr = {
    ...
    setPortPathCost: (target, cost) => cost === null
    ...
    setPortEdgeMode: (target, mode, previous) => {
        if (mode)
            return buildInterfaceCommand(target, `spanning-tree ${mode}`);
        return buildInterfaceCommand(target, `no spanning-tree ${previous ?? "edgeport"}`);
    },
    setPortBpduGuard: (target, enabled) => buildInterfaceCommand(target, `spanning-tree bpdu-guard ${enabled ? "enable" : "disable"}`),
};
