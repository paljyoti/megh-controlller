...
export const loopDetectTr = {
    setGlobalEnabled: (enabled) => ({
        command: enabled ? "loop-detect enable" : "no loop-detect enable",
        mode: "config",
    }),
    setPortEnabled: (target, enabled) => buildInterfaceCommand(target, enabled ? "loop-detect enable" : "no loop-detect enable"),
    setPortAction: (target, action) => buildInterfaceCommand(target, `loop-detect action ${action}`),
    setPortVlans: (target, vlanSpec) => vlanSpec
